...
export async function exportLedger(
  ...
  prefiltered?: [JournalEntry, LineItem[]][],
): Promise<string> {
  let out = "; Generated by dledger\n\n";

  // Compute entries first so we can derive used accounts/currencies for filtered exports
  const entries = prefiltered ?? await backend.queryJournalEntries({});
  const sortedEntries = [...entries].sort((a, b) =>
    a[0].date.localeCompare(b[0].date),
  );

  const usedAccountIds = prefiltered
    ? new Set(entries.flatMap(([, items]) => items.map(i => i.account_id)))
    : null;
  ...
  for (const acc of sorted) {
    if (usedAccountIds && !usedAccountIds.has(acc.id)) continue;
    if (format === "hledger") {
  ...
  const usedCurrencies = prefiltered
    ? new Set(entries.flatMap(([, items]) => items.map(i => i.currency)))
    : null;
  const minDate = prefiltered ? sortedEntries[0]?.[0].date : null;
  const maxDate = prefiltered ? sortedEntries.at(-1)?.[0].date : null;

  for (const rate of sortedRates) {
    if (usedCurrencies) {
      if (!usedCurrencies.has(rate.from_currency) && !usedCurrencies.has(rate.to_currency)) continue;
      if (minDate && maxDate && (rate.date < minDate || rate.date > maxDate)) continue;
    }

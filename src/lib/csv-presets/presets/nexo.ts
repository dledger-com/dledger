    ...
    for (const row of rows) {
      ...
      if (typeUpper === "WITHDRAWAL") {
        ...
        if (inCurr && outCurr && !isNaN(inAmt) && !isNaN(outAmt)) {
          ...
          lines.push(
            { account: `Assets:Nexo:${inCurr}`, currency: inCurr, amount: (-absIn).toString() },
            { account: "Equity:Trading", currency: inCurr, amount: absIn.toString() },
            { account: `Assets:Nexo:${outCurr}`, currency: outCurr, amount: absOut.toString() },
        ...
        if (curr && amt > 0) {
          lines.push(
            { account: `Assets:Nexo:${curr}`, currency: curr, amount: amt.toString() },
            { account: "Income:Nexo:Interest", currency: curr, amount: (-amt).toString() },
          );
        }
        ...
        if (curr && amt > 0) {
          lines.push(
            { account: `Assets:Nexo:${curr}`, currency: curr, amount: amt.toString() },
            { account: "Income:Nexo:Cashback", currency: curr, amount: (-amt).toString() },
          );
        }

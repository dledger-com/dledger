...
export async function syncBittensorAccount(
	...
	async function ensureCurrency(code: string): Promise<void> {
		...
		await backend.createCurrency({ code, asset_type: "", param: "", name: code, decimal_places: TAO_DECIMALS, is_base: false });
	...
	for (const tx of transfers) {
		...
		if (direction === "sent") {
			lineItemData.push(
				...
				{ account: walletExternal(CHAIN, shortAddr(counterparty)), currency: "TAO", amount: amount },
			);
		} else if (direction === "received") {
			lineItemData.push(
				{ account: walletAssets(CHAIN, account.label), currency: "TAO", amount: amount },
				{ account: walletExternal(CHAIN, shortAddr(counterparty)), currency: "TAO", amount: new Decimal(amount).neg().toFixed() },
			);
		} else {
			lineItemData.push(
				{ account: walletAssets(CHAIN, account.label), currency: "TAO", amount: "0" },
				{ account: walletExternal(CHAIN, shortAddr(counterparty)), currency: "TAO", amount: "0" },
			);
		}

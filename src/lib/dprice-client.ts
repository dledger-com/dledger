...
class HttpDpriceClient implements DpriceClient {
  ...
  async getPriceRange(
    ...
    const query = `{ priceHistory(symbols: ["${symbol}"], from: "${fromDate}", to: "${toDate}") { currencies { prices { date priceUsd } } } }`;
    ...
    const currencies = json.data.priceHistory.currencies as Array<{ prices: Array<{ date: number; priceUsd: string }> }>;
    if (currencies.length === 0) return [];
    return currencies[0].prices.map(
      (p) => ({ date: String(p.date), price_usd: p.priceUsd }),
    );
  }

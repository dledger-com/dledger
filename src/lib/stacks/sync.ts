// Stacks sync — fetch transactions via Hiro API and create journal entries.
...
function microToStx(micro: string): string {
	return new Decimal(micro).div(new Decimal(10).pow(STX_DECIMALS)).toFixed();
}
...
export async function syncStacksAccount(
	...
	for (const [e] of await backend.queryJournalEntries({})) {
		if (e.source.startsWith("stacks:")) existingSources.add(e.source);
	}
	...
	async function ensureCurrency(code: string, decimals?: number): Promise<void> {
		...
		await backend.createCurrency({ code, asset_type: assetType, param: "", name: code, decimal_places: decimals ?? STX_DECIMALS, is_base: false });
	...
	await ensureCurrency("STX");

	// 3. Process transactions
	for (let i = 0; i < transactions.length; i++) {
		if (signal?.aborted) throw new DOMException("Aborted", "AbortError");
		const tx = transactions[i];
		const source = `stacks:${tx.tx_id}`;
		...
		const date = new Date(tx.burn_block_time * 1000).toISOString().slice(0, 10);

		// Process token transfers
		if (tx.tx_type === "token_transfer" && tx.token_transfer) {
			const transfer = tx.token_transfer;
			const amount = microToStx(transfer.amount);
			const fee = microToStx(tx.fee_rate);
			...
			const isSender = tx.sender_address === account.address;
			const isReceiver = transfer.recipient_address === account.address;
			const direction: "sent" | "received" | "self" = isSender && isReceiver ? "self" : isSender ? "sent" : "received";
			const counterparty = isSender ? transfer.recipient_address : tx.sender_address;
			...
			if (isSender && new Decimal(fee).gt(0)) {
				lineItemData.push(
					{ account: chainFees(CHAIN), currency: "STX", amount: fee },
					{ account: walletAssets(CHAIN, account.label), currency: "STX", amount: new Decimal(fee).neg().toFixed() },
				);
			}

			const descData = onchainTransferDescription(CHAIN, "STX", direction, { txHash: tx.tx_id });
			...
			try {
				...
				result.warnings.push(`post ${tx.tx_id.slice(0, 12)}: ${e instanceof Error ? e.message : String(e)}`);
			}
		} else {
			// Skip contract_call and other types for now
			result.transactions_skipped++;
		}
	}

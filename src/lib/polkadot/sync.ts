...
import type { PolkadotSyncResult, SubscanTransfer, SubscanReward } from "./types.js";
...
export async function syncPolkadotAccount(
	...
	async function ensureCurrency(code: string): Promise<void> {
		await ensureCurrencyExists(backend, code, currencySet, { context: "crypto-chain", decimals: 10 });
	}

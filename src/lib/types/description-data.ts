export type DescriptionData =
  ...
  | { type: "fee"; chain: string; currency: string; txHash: string }
...
/** On-chain transfer (EVM): "Send ETH to 0x..." */
...
/** DeFi protocol action: "Uniswap: Swap WETH → USDC" */
...
/** Bitcoin transfer: "Send BTC to bc1q..." */
...
/** Solana transfer: "Send SOL to abc..." */
...
/** Solana DeFi action: "Jupiter: Swap SOL → USDC" */
...
/** Perpetual futures trade: "Hyperliquid: BTC Long" */
...
export function feeDescription(chain: string, currency: string, txHash: string): DescriptionData {
...
export function renderDescription(data: DescriptionData): string {
  switch (data.type) {
    ...
    case "cex-transfer":
      return `${data.exchange}: ${capitalize(data.direction)} ${data.currency}`;
    case "cex-reward":
      return `${data.exchange}: ${capitalize(data.kind)} reward ${data.currency}`;
    case "cex-operation":
      return `${data.exchange}: ${capitalize(data.operation)} ${data.currency}`;
    case "bank":
    ...
    case "hl-fill": {
      ...
      return `Hyperliquid: ${data.coin} ${capitalize(data.side)}`;
    }
    case "hl-funding":
      return `Hyperliquid: Funding ${data.coin}`;
    case "hl-ledger":
      return `Hyperliquid: ${capitalize(data.action)} USDC`;
    case "generic-import":
      return data.text;
    case "manual":
      return data.text;
